...
#include "pdcsdl.h"
extern void VirtualKeyboard(bool pressed);
...
static int _process_mouse_event(void) {
    ...
    if (event.type == SDL_MOUSEMOTION) {
        ...
        pdc_mouse_status.x = (int) (event.tfinger.x * pdc_fwidth);
        pdc_mouse_status.y = (int) (event.tfinger.y * pdc_fheight);
...
int PDC_get_key(void) {
    switch (event.type) {
        ...
        case SDL_JOYBUTTONDOWN:
            fprintf(stderr,"Joystick %d button %d down\n", event.jbutton.which, event.jbutton.button);
        case SDL_KEYUP:
        case SDL_KEYDOWN:
        case SDL_TEXTINPUT:
            PDC_mouse_set();
